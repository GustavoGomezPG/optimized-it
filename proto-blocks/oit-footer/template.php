...
<footer <?php echo $wrapper_attributes; ?>>
  <div class="oit-footer bg-black text-white px-4 pt-12 pb-8 lg:px-20 lg:pt-16 lg:pb-8">
    <div class="max-w-[1360px] mx-auto flex flex-col gap-12">

      <?php if ($show_newsletter_strip): ?>
      <div
        class="oit-footer__newsletter bg-slate rounded-[24px] flex flex-col lg:flex-row items-stretch lg:items-center justify-between gap-4 lg:gap-8 px-6 py-5 lg:px-6 lg:py-4">
        <?php if (!empty($newsletter_heading)): ?>
        <h2 class="font-grotesk font-bold text-[20px] leading-[1.4] uppercase whitespace-nowrap m-0">
      ...
      <div class="grid grid-cols-1 lg:grid-cols-[max-content_1fr] gap-12 lg:gap-[80px]">
        <div class="flex flex-col gap-6 lg:max-w-[329px]">
          <div class="flex flex-col gap-2">
            <a href="<?php echo esc_url(home_url('/')); ?>" class="no-underline self-start"
              aria-label="<?php echo esc_attr(get_bloginfo('name')); ?>">
              <?php if (!empty($logo['url'])): ?>
              <img data-proto-field="logo" src="<?php echo esc_url($logo['url']); ?>"
                alt="<?php echo esc_attr($logo['alt'] ?? get_bloginfo('name')); ?>" class="h-[73px] w-auto" />
              <?php else: ?>
            ...
            <p class="font-dm font-medium text-[14px] leading-[1.5] m-0">
          ...
          <a href="<?php echo esc_url($cta_button['url'] ?? '#'); ?>"
            ...
            class="oit-nav__cta no-underline self-start inline-flex items-center gap-3 bg-brand-red hover:bg-brand-red-700 text-white font-grotesk font-medium text-[16px] leading-[1.3] uppercase px-5 py-2.5 rounded-full transition-colors whitespace-nowrap"
          ...
          <div class="oit-footer__contact flex flex-col gap-2">
            ...
            <a href="tel:<?php echo esc_attr(preg_replace('/[^0-9+]/', '', $phone_number)); ?>"
              class="oit-footer__phone no-underline inline-flex items-center gap-2 self-start border-b-2 border-brand-red pt-1 pb-2 text-white font-dm font-medium text-[16px] leading-[1.5]">
              <svg width="18" height="17" viewBox="0 0 18 17" fill="currentColor" aria-hidden="true"
      ...
      <div class="flex flex-col gap-6">
        <hr class="border-0 border-t-2 border-brand-red m-0" />
